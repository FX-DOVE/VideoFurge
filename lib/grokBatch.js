...
function safeResolve(p) {
  ...
  try { return path.resolve(p); } catch (_) { return null; }
}

function buildStylePrompt(referenceImagePaths, characters = [], sceneStyle = '', styleReferencePaths = []) {
  // Attach character images (skip null/missing paths — drama-safe)
  ...
  const styleList = styleReferencePaths && styleReferencePaths.length
    ? styleReferencePaths.map(p => safeResolve(p)).filter(Boolean).map(abs => `@${abs}`).join(' ')
    : '';
...
async function runBatch(batch, styleSummary, batchIndex, jobDir, extra = {}) {
  ...
  // Mode + aspect come from the first item (buildPrompts injects them per beat).
  const first = batch[0] || {};
  const modeEmphasis = first.modeEmphasis || '';
  const videoType = first.videoType || 'documentary';
  const aspect = resolveAspect(first.resolution || '16:9');
  ...
  if (characters && characters.length > 0) {
    characters.forEach((c) => {
      if (c.imagePath) {
        const abs = safeResolve(c.imagePath);
        if (abs) charLines.push(`- Character "${c.name}" (${c.gender || 'unspecified'}): @${abs}`);
      }
    });
  }
  ...
  if (styleReferencePaths && styleReferencePaths.length > 0) {
    styleReferencePaths.forEach((p, i) => {
      ...
      if (abs) styleLines.push(`- Style / Environment / Scene ref ${i + 1}: @${abs}`);
    });
  }
  ...
  const taskLines = pending.map((item, i) => {
    ...
    // Dynamic Character Context — now includes rich acting/performance direction.
    let charContext = '';
    if (item.resolvedCharacters && item.resolvedCharacters.length > 0) {
      charContext = '\nCHARACTER DETAILS FOR THIS BEAT:\n' + item.resolvedCharacters.map(rc => {
        ...
        const p = rc.performance;
        if (p) {
          if (p.emotion && p.emotion.length)          lines += `\n  PERFORMANCE emotion: ${p.emotion.join('; ')}`;
          if (p.facialExpression && p.facialExpression.length) lines += `\n  FACE: ${p.facialExpression.join('; ')}`;
          if (p.bodyLanguage && p.bodyLanguage.length) lines += `\n  BODY: ${p.bodyLanguage.join('; ')}`;
          if (p.eyeDirection)                          lines += `\n  EYES: ${p.eyeDirection}`;
          if (p.voicePerformance)                      lines += `\n  VOICE: ${p.voicePerformance}`;
          if (p.interaction)                           lines += `\n  INTERACTION: ${p.interaction}`;
        }
        if (rc.canonicalRefPaths && rc.canonicalRefPaths.length > 0) {
          const cref = safeResolve(rc.canonicalRefPaths.find(p => typeof p === 'string' && p.trim()));
          if (cref) lines += `\n  Canonical Ref: @${cref}`;
        }
        return lines;
      }).join('\n') + '\n';
    ...
    // Dialogue performance block (drama / movie modes).
    let dialogueBlock = '';
    if (Array.isArray(item.dialogue) && item.dialogue.length > 0) {
      dialogueBlock = '\nDIALOGUE PERFORMANCE (characters must appear to speak these lines with natural mouth movement):\n' +
        item.dialogue.map(d => {
          let l = `  ${d.speaker || 'Speaker'}: "${d.line || ''}"`;
          if (d.performance && d.performance.length) l += `\n    delivery: ${d.performance.join('; ')}`;
          if (d.listenerReaction) l += `\n    listener reaction: ${d.listenerReaction}`;
          if (d.emotionTransition) l += `\n    emotion transition: ${d.emotionTransition}`;
          return l;
        }).join('\n') + '\n';
    }
    ...
    // Semantic intensity replaces the old keyword drama scan.
    const intensity = (typeof item.intensity === 'number') ? item.intensity : 0;
    const hasDrama = intensity >= 0.45 || (Array.isArray(item.dialogue) && item.dialogue.length > 0);

    let dramaBlock = '';
    if (hasDrama) {
      dramaBlock =
        `  DRAMA & ACTING OVERRIDE (emotional intensity ${intensity.toFixed(2)}):\n` +
        `  - This is a key emotional/dramatic moment. DO NOT make it a generic or static pose.\n` +
        `  - The characters must perform realistic, highly expressive acting: clear facial expressions, motivated body language, eye contact/aversion, and (if dialogue is present) natural speaking with matching mouth movement.\n` +
        `  - Compose it like a shot from a real high-end drama or film, capturing the emotion, relationships, and conflict so a viewer understands the moment WITHOUT narration.\n`;
    }
    ...
    return (
      ...
      `  Create a frame that VISUALLY ACTS OUT the exact situation, actions, expressions, and relationships described above.\n` +
      (dramaBlock ? `${dramaBlock}` : '') +
      `  - Combine and reference the attached character images + style/environment images as needed to make the single best image for this specific beat.\n` +
      ...
      `Step B — Animate to video with full PERFORMANCE direction:\n` +
      `  Use the animate / video_gen tool on the image you just created.\n` +
      (motionLine ? `${motionLine}\n` : '') +
      `  CHARACTER PERFORMANCE: each character must move and emote as described above — facial motion, body movement, eye contact, and (for dialogue) natural mouth movement while speaking, with the listener visibly reacting.\n` +
      `  EMOTIONAL TRANSITION: let the performance evolve across the clip rather than a single frozen pose.\n` +
      `  The resulting clip must feel like real actors performing the moment, in the exact same visual style as the reference images.\n` +
      (hasDrama ? `  - High-intensity beat: the animation MUST show real physical acting, expressive motion, and (if speaking) talking, not subtle drift.\n` : '') +
  ...
  const fullPrompt =
    `You are an expert AI Movie Director and Producer working in "${videoType}" mode.\n` +
    `Your goal: visuals so compelling that the audience understands the story just by WATCHING.\n` +
    (modeEmphasis ? `MODE DIRECTION: ${modeEmphasis}\n` : '') +
    `Target aspect ratio: ${aspect.ratio} (${aspect.w}x${aspect.h}). Compose every shot for this frame.\n` +
