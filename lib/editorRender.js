...
function normalizeTimeline(timeline, clips) {
  ...
  return { version: 1, clips: outClips, audioMode, defaultTransition, resolution, captions };
}
...
async function renderEditedVideo({ jobDir, job, timeline, onProgress }) {
  ...
  if (plan.audioMode === 'silent' || !audioPath) {
    const args = ['-i', concatVideo, '-filter_complex', vScale, '-map', '[v]'];
    if (hasVidAudio && plan.audioMode !== 'silent') {
    ...
    await runFfmpeg(args, 'editor mux silent');
  } else {
    const args = ['-i', concatVideo, '-i', audioPath];
    if (hasVidAudio) {
      args.push(
        '-filter_complex',
    ...
    await runFfmpeg(args, 'editor mux audio');
  }
