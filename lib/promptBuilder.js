...
// ─── Prompt builder ────────────────────────────────────────────────────────
function buildPrompts(beats, styleSummary, scriptContext) {
  return beats.map((beat, i) => {
    const mood        = detectMood(beat.text);
    const sceneType   = detectSceneType(beat.text);
    const shot        = SHOT_TYPES[i % SHOT_TYPES.length];
    const motion      = CAMERA_MOTIONS[i % CAMERA_MOTIONS.length];
    const lighting    = MOOD_LIGHTING[mood];
    const atmosphere  = MOOD_ATMOSPHERE[mood];
